  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-center">
          <p className="text-sm font-semibold capitalize">
            {JOURS[current.getDay()]} {current.getDate()} {MOIS[current.getMonth()]}{" "}
            {current.getFullYear()}
          </p>
                ...
                <div className="flex flex-wrap gap-2">
                  {DEPARTMENTS.map((dept) => {
                    ...
                    return (
                      <div
                        key={dept.label}
                        className={cn("min-w-[80px] rounded-lg px-2.5 py-2", dept.bg)}
                      >
                        <p
                          className={cn(
                            "mb-1 text-[9px] font-bold tracking-wide uppercase opacity-60",
                            dept.text
                          )}
                        >
                          {dept.label}
                        ...
                        {group.map((emp) => (
                          <p
                            key={emp.id}
                            className={cn("text-[11px] leading-snug font-medium", dept.text)}
                          >
                            {emp.name.split(" ")[0]}
                          </p>
                        ))}
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-muted-foreground/50 text-[10px] font-light tracking-widest">
          coupure
        </span>
        <div className="bg-border h-px flex-1" />
      </div>
