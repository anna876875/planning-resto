  ...
  return (
    <div className="flex min-h-0 flex-1 overflow-auto">
      <table
        ...
        style={{ minWidth: `${180 + days.length * 120}px` }}
      >
        {/* ── En-têtes jours ── */}
        <thead>
            ...
            {days.map((d) => {
              ...
              return (
                <th
                  key={d}
                  className={cn(
                    "text-muted-foreground px-3 pt-3 pb-2.5 text-center text-[10px] font-semibold tracking-widest uppercase",
              ...
              >
                ...
                <td className={cn("py-4 align-middle", svc.rowBg)} style={{ width: 64 }}>
                  <div className="flex flex-col items-center justify-center gap-1.5 px-2">
                    <span
                      className={cn(
                        "text-[11px] font-light tracking-wide tabular-nums select-none",
                        svc.text
                      )}
                    >
                      {svc.startH}
                    </span>
                    <span className={cn("h-1.5 w-1.5 shrink-0 rounded-full", svc.dot)} />
                  </div>
                </td>
                ...
                {days.map((d) => {
                  ...
                  const working = employees.filter((emp) =>
                    shifts.some(
                      (s) => s.employeeId === emp.id && s.date === d && s.type === svc.key
                    )
                  );
                  return (
                    <td
                        ...
                        {DEPARTMENTS.map((dept) => {
                          ...
                          return (
                            <div key={dept.label} className={cn("rounded px-1.5 py-1", dept.bg)}>
                              <span
                                className={cn(
                                  "mb-0.5 block text-[9px] font-bold tracking-wide uppercase opacity-70",
                                  dept.text
                                )}
                              >
                                {dept.label}
                              ...
                              {group.map((emp) => (
                                <span
                                  key={emp.id}
                                  className={cn(
                                    "block text-[11px] leading-snug font-medium whitespace-nowrap",
                                    dept.text
                                  )}
                                >
                                  {emp.name.split(" ")[0]}
