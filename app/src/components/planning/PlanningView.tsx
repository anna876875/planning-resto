...
const DEPARTMENTS = [
  {
    label: "Cuisine",
    roles: ["chef_cuisine", "chef_partie"],
    bg: "bg-orange-100/80",
...
type ConflictCode =
  "heures_depassees" | "jours_consecutifs" | "repos_insuffisant" | "sous_effectif";
interface Conflict {
  code: ConflictCode;
...
function detect(shifts: Shift[], weekDays: Date[]) {
  ...
  employees.forEach((emp) => {
    ...
    for (let i = 1; i < wd.length; i++) {
      ...
      if (streak >= 6)
        all.push({
          ...
          level: "avertissement",
          message: `${emp.name.split(" ")[0]} — ${streak} jours d'affilée`,
          employeeId: emp.id,
          date: wd[i],
        });
    }
    ...
    const sorted = es
      .filter((s) => s.start && s.end && s.type !== "repos")
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (rest >= 0 && rest < 11)
        all.push({
          ...
          level: "bloquant",
          message: `${emp.name.split(" ")[0]} — seulement ${Math.round(rest)}h de repos`,
          employeeId: emp.id,
          date: sorted[i].date,
        });
    }
    ...
    weekDays.forEach((day) => {
      ...
      if (count > 0 && count < svc.min)
        all.push({
          code: "sous_effectif",
          level: "bloquant",
          message: `${DAY_SHORT[weekDays.indexOf(day)]} ${svc.label} — ${count} pers. (min. ${svc.min})`,
          date: dateStr,
        });
    })
  );
...
export function PlanningView({
    ...
    () =>
      SERVICE_ROWS.map((svc) => ({
        ...
        days: weekDays.map((day) => {
          ...
          return {
            dateStr,
            working: employees.filter((emp) =>
              shifts.some(
                (s) => s.employeeId === emp.id && s.date === dateStr && s.type === svc.type
              )
            ),
          };
        }),
  ...
  const markModified = () => {
    if (published === "publié") setStatus("modifié");
  };
  ...
  function quickAdd(emp: Employee, date: string, type: ServiceType) {
    ...
    setShiftsMap((prev) => {
      ...
      return {
        ...prev,
        [weekKey]: [...cur.filter((x) => !(x.employeeId === emp.id && x.date === date)), s],
      };
    });
    markModified();
  }
  ...
  function removeEmp(emp: Employee, date: string) {
    setShiftsMap((prev) => {
      ...
      return {
        ...prev,
        [weekKey]: cur.filter((s) => !(s.employeeId === emp.id && s.date === date)),
      };
    });
    markModified();
  }
  ...
  function saveShift(data: Omit<Shift, "id"> & { id?: string }) {
    ...
    setShiftsMap((prev) => {
      ...
      return {
        ...prev,
        [weekKey]: [...cur.filter((x) => !(x.employeeId === s.employeeId && x.date === s.date)), s],
      };
    });
    markModified();
  }
  ...
  function toggleLock() {
    setStatus((s) => (s === "verrouillé" ? (published ?? "brouillon") : "verrouillé"));
  }
  function copyWeek() {
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        ...
        {!hideNav ? (
          <div className="flex flex-1 items-center justify-center gap-1">
            <Button
              ...
              className="h-7 w-7"
              onClick={() =>
                viewMode === "semaine" ? setWeekOffset((w) => w - 1) : setDayOffset((d) => d - 1)
              }
            >
            ...
            <div className="border-border ml-2 flex overflow-hidden rounded-md border">
          ...
          {readOnly ? (
            <>
              <Link
                ...
                className="border-input bg-background hover:bg-accent hover:text-accent-foreground inline-flex h-8 items-center gap-1.5 rounded-md border px-3 text-sm font-medium shadow-sm transition-colors"
              >
            ...
            <>
              ...
              <Button
                ...
                disabled={locked || bloquants.length > 0}
              >
        ...
        >
          ...
          <div>
            ...
            <span className="text-muted-foreground ml-1.5 opacity-70">
              — survolez les icônes pour les détails.
            </span>
          </div>
        </div>
      )}
      ...
      {activeTab === "planning" && viewMode === "jour" && (
        <div className="flex-1 space-y-3 overflow-y-auto p-4">
          {SERVICE_ROWS.map((svc) => {
            ...
            return (
              <div key={svc.type} className="border-border overflow-hidden rounded-xl border">
                <div className={cn("flex items-center gap-2 px-4 py-2.5", svc.chip)}>
                  <span className={cn("h-2 w-2 shrink-0 rounded-full", svc.dot)} />
                ...
                {working.length === 0 ? (
                  <p className="text-muted-foreground px-4 py-3 text-xs italic">Aucun service</p>
                ) : (
                  <div className="bg-background flex flex-wrap gap-2 p-3">
                    {DEPARTMENTS.map((dept) => {
                      ...
                      return (
                        <div
                          key={dept.label}
                          className={cn("min-w-[120px] rounded-lg px-3 py-2", dept.bg)}
                        >
                          <p className={cn("mb-1 text-[10px] font-semibold", dept.text)}>
                            {dept.label}
                          </p>
                          {group.map((emp) => (
                            <p
                              key={emp.id}
                              className={cn("text-[12px] leading-snug font-medium", dept.text)}
                            >
                              {emp.name.split(" ")[0]}
                            </p>
                          ))}
                            ...
                            {working.map((emp) => (
                              ...
                              >
                                <button
                                  type="button"
                                  onClick={() =>
                                    setEditModal({ employeeId: emp.id, date: dateStr })
                                  }
                                ...
                                <button
                                  ...
                                  className="ml-auto shrink-0 rounded opacity-0 transition-opacity group-hover:opacity-60 hover:text-red-600 hover:!opacity-100"
                                >
                                  <X className="h-3 w-3" />
                                </button>
                              </div>
                        ...
                        working.length > 0 ? (
                          showNames ? (
                            ...
                            <div className="group/cell relative inline-block">
                              <span
                                className={cn(
                                  "mx-auto flex h-8 w-8 cursor-default items-center justify-center rounded-md text-sm font-bold transition-colors select-none",
                                  isToday ? "bg-primary/15 text-primary" : svc.chip
                                )}
                              >
                                {working.length}
                              </span>
                              <div className="border-border bg-background pointer-events-none invisible absolute top-full left-1/2 z-50 mt-1.5 min-w-[120px] -translate-x-1/2 rounded-md border px-2.5 py-2 text-left shadow-md group-hover/cell:visible">
                                <p
                                  className={cn(
                                    "mb-1.5 text-[10px] font-semibold tracking-wide uppercase",
                                    svc.text
                                  )}
                                >
                                  {svc.label} · {working.length} pers.
                                </p>
                                {working.map((emp) => (
                                  <p
                                    key={emp.id}
                                    className="text-foreground text-xs leading-relaxed whitespace-nowrap"
                                  >
                                    {emp.name}
                                  </p>
                                ))}
      ...
      {shareOpen && (
        <>
          ...
          />
          <div className="border-border bg-background fixed top-1/2 left-1/2 z-50 w-[min(26rem,92vw)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border shadow-2xl">
            {/* Header */}
            ...
            <div className="max-h-72 overflow-y-auto px-5 py-3">
              <div className="text-muted-foreground mb-2.5 flex items-center justify-between text-[11px] font-semibold tracking-wide uppercase">
                <span>
                  Destinataires ({selected.size}/{employees.length})
                </span>
                <button
                  type="button"
                  onClick={() =>
                  ...
                  className="text-primary font-medium tracking-normal normal-case hover:underline"
                >
